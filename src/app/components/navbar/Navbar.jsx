    ...
    <>
      <header data-aos="fade-down" className={styles.header}>
        <div className={styles.container}>
          <div className={styles.inner} style={{ height: 76 }}>
            <div className={styles.brandSection} style={{ zIndex: 3 }}>
              <a
                role="button"
                href="#"
                onClick={(e) => {
              ...
              >
                <img src={`${base}logo.svg`} alt="logo" width={36} height={36} />
                <div className={styles.brandText}>Enterprei</div>
              </a>
            </div>

            <nav data-aos="fade-down" className={styles.centerNav}>
              {LINKS.map((l) => (
                l.children && l.children.length ? (
                  <div
                    key={l.label}
                    className={`${styles.dropdown} ${openMenu === l.label ? styles.open : ''}`}
                    onMouseEnter={() => {
                    ...
                    <div className={styles.dropdownMenu} role="menu" aria-hidden={openMenu !== l.label}>
                      ...
                      <ul className={styles.dropdownList}>
                        {l.children.map((c, i) => {
                          ...
                          const iconSrc = c.icon ? `${base}${c.icon.replace(/^\/+/, '')}` : null;
                          return (
                            <a key={i} href={href} className={styles.dropdownItem} onClick={() => setOpenMenu(null)}>
                              {c.icon ? (
                                <img src={iconSrc} alt={c.title} className={styles.dropdownIcon} />
                              ) : (
                  ...
                  <a key={l.label} className={styles.navLink} href={l.href}>{l.label}</a>
                )
              ))}
            </nav>
